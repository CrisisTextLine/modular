...
func TestPrometheusCollectorBasic(t *testing.T) {
	...
	if err := eb.RegisterConfig(app); err != nil { t.Fatalf("register config: %v", err) }
	if err := eb.Init(app); err != nil { t.Fatalf("init module: %v", err) }
	ctx := context.Background()
	if err := eb.Start(ctx); err != nil { t.Fatalf("start module: %v", err) }
	...
	// Publish some events
	for i := 0; i < 5; i++ {
		if err := eb.Publish(ctx, "metric.test", i); err != nil { t.Fatalf("publish: %v", err) }
	}
	...
	if len(metrics) == 0 { t.Fatalf("expected metrics gathered") }
		...
		if m.GetName() == "modular_eventbus_test_delivered_total" {
			for _, mm := range m.GetMetric() {
				...
				for _, l := range mm.GetLabel() {
					if l.GetName() == "engine" { engineLabel = l.GetValue() }
				}
				if engineLabel == "_all" {
	...
	if !found { t.Fatalf("did not find aggregate delivered metric") }
