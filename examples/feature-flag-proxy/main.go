...
func main() {
	...
	// Register beta tenant with specific configuration
	err := tenantService.RegisterTenant("beta-tenant", map[string]modular.ConfigProvider{
		"reverseproxy": modular.NewStdConfigProvider(&reverseproxy.ReverseProxyConfig{
			TenantIDHeader: "X-Tenant-ID",
			DefaultBackend: "beta-backend",
			BackendServices: map[string]string{
				"beta-backend": "http://localhost:9005", // Special backend for beta tenant
			},
		}),
	})
	if err != nil {
		app.Logger().Error("Failed to register beta tenant", "error", err)
		os.Exit(1)
	}
