...
func TestMemoryEventBusConcurrentPublishSubscribe(t *testing.T) {
    ...
    if err := module.Init(app); err != nil { t.Fatalf("init: %v", err) }
    ctx := context.Background()
    if err := module.Start(ctx); err != nil { t.Fatalf("start: %v", err) }
    defer module.Stop(ctx)

    var asyncCount, syncCount int64
    for i := 0; i < asyncSubs; i++ {
        if _, err := module.SubscribeAsync(ctx, topic, func(ctx context.Context, e Event) error { atomic.AddInt64(&asyncCount, 1); return nil }); err != nil { t.Fatalf("async sub: %v", err) }
    }
    for i := 0; i < syncSubs; i++ {
        if _, err := module.Subscribe(ctx, topic, func(ctx context.Context, e Event) error { atomic.AddInt64(&syncCount, 1); return nil }); err != nil { t.Fatalf("sync sub: %v", err) }
    }
    ...
    for p := 0; p < publisherCount; p++ {
        go func() {
            defer wg.Done()
            for i := 0; i < messagesPerPub; i++ { _ = module.Publish(ctx, topic, payload) }
        }()
    }
    wg.Wait()
    ...
    if finalSync == 0 || finalAsync == 0 { t.Fatalf("expected deliveries sync=%d async=%d", finalSync, finalAsync) }
    ratio := float64(finalAsync) / float64(finalSync)
    if ratio < 0.10 { t.Fatalf("async severely starved ratio=%.3f sync=%d async=%d", ratio, finalSync, finalAsync) }
}
...
func TestMemoryEventBusBlockingModeFairness(t *testing.T) {
    ...
    var asyncCount, syncCount int64
    for i := 0; i < asyncSubs; i++ { if _, err := module.SubscribeAsync(ctx, topic, func(ctx context.Context, e Event) error { atomic.AddInt64(&asyncCount, 1); return nil }); err != nil { t.Fatalf("async sub: %v", err) } }
    for i := 0; i < syncSubs; i++ { if _, err := module.Subscribe(ctx, topic, func(ctx context.Context, e Event) error { atomic.AddInt64(&syncCount, 1); return nil }); err != nil { t.Fatalf("sync sub: %v", err) } }
    ...
    if ratio < 0.25 || ratio > 3.0 { t.Fatalf("unfair distribution ratio=%.2f sync=%d async=%d", ratio, finalSync, finalAsync) }
}
...
func TestMemoryEventBusUnsubscribeDuringPublish(t *testing.T) {
    ...
    if err := module.Unsubscribe(ctx, sub); err != nil { t.Fatalf("unsubscribe: %v", err) }
    close(stopPub)
    <-done
    final := atomic.LoadInt64(&count)
    if final == 0 { t.Fatalf("expected some deliveries before unsubscribe") }
    time.Sleep(50 * time.Millisecond)
    if atomic.LoadInt64(&count) != final { t.Fatalf("deliveries continued after unsubscribe") }
}
...
func TestMemoryEventBusStatsAccounting(t *testing.T) {
    ...
    if _, err := module.SubscribeAsync(ctx, topic, func(ctx context.Context, e Event) error {
        ...
        return nil
    }); err != nil { t.Fatalf("subscribe: %v", err) }

    publishCount := 1000
    payload := map[string]any{"n": 1}
    for i := 0; i < publishCount; i++ { _ = module.Publish(ctx, topic, payload) }
    // Allow processing/drain
    time.Sleep(500 * time.Millisecond)

    delivered, dropped := module.Stats()
    totalAccounted := delivered + dropped
    if totalAccounted == 0 { t.Fatalf("expected some accounted events") }
    if totalAccounted > uint64(publishCount) { t.Fatalf("accounted exceeds publishes accounted=%d publishes=%d", totalAccounted, publishCount) }
    // Delivered should match recv
    if delivered != uint64(atomic.LoadInt64(&recv)) { t.Fatalf("delivered mismatch stats=%d recv=%d", delivered, recv) }
    // Basic ratio sanity: at least some delivered
    if delivered == 0 { t.Fatalf("no delivered events recorded") }
}
